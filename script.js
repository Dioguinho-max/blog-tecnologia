...
function renderAccountPopover() {
    if (readerSession?.user) {
        accountPopover.innerHTML = `<strong>${readerSession.user.email}</strong><span>Conta conectada</span><details><summary>Configurações</summary><form id="popover-settings"><label>Nova foto<input id="popover-avatar" type="file" accept="image/*"></label><label>Novo e-mail<input name="email" type="email" placeholder="Novo e-mail"></label><label>Nova senha<input name="password" type="password" minlength="6" placeholder="Nova senha"></label><button type="submit">Salvar alterações</button></form></details><button id="popover-logout" type="button">Sair da conta</button>`;
