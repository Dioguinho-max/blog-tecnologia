...
function showAdmin() { loginPanel.hidden = true; adminPanel.hidden = false; document.querySelector("#admin-comments").hidden = false; document.querySelector("#admin-overview").hidden = false; restoreDraft(); loadPosts(); loadComments(); loadStats(); }
async function loadStats() { const response = await fetch(`${api}/posts/admin/stats`, { headers: authHeaders() }); if (!response.ok) return; const stats = await response.json(); document.querySelector("#admin-stats").innerHTML = `<article><strong>${stats.published}</strong><span>publicados</span></article><article><strong>${stats.drafts}</strong><span>rascunhos</span></article><article><strong>${stats.views}</strong><span>leituras</span></article><article><strong>${stats.pending_comments}</strong><span>comentários pendentes</span></article>${stats.top_post ? `<p>Mais lido: <strong>${escapeHtml(stats.top_post)}</strong></p>` : ""}`; const notice = document.querySelector("#admin-notice"); notice.textContent = stats.pending_comments ? `Você tem ${stats.pending_comments} comentário(s) aguardando moderação.` : ""; notice.hidden = !stats.pending_comments; }
